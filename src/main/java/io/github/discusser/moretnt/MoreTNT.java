...
public class MoreTNT {
    ...
    public void commonSetup(FMLCommonSetupEvent event) {
        for (MoreTNTObject object : MoreTNTObjects.objects) {
            ...
            DispenserBlock.registerBehavior(object.blockItem().item().get(), this.dispenserExecute(
                    (level, blockPos, stack) -> block.createPrimedFromStack(level, stack, blockPos, block.size,
                            block.fire)));
        }
    }


    public DefaultDispenseItemBehavior dispenserExecute(DispenserConsumer function) {
        return new DefaultDispenseItemBehavior() {
            ...
            protected @NotNull ItemStack execute(@NotNull BlockSource pSource, @NotNull ItemStack pStack) {
                ...
                BasePrimedTNT instance = function.createPrimed(level, blockpos, pStack);
    ...
    public interface DispenserConsumer {
        BasePrimedTNT createPrimed(Level level, BlockPos blockPos, ItemStack pStack);
    }
}
